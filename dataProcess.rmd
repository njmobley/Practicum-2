...
medianBalance = median(raw_data$balance)
medianCampaign = median(raw_data$campaign)
medianAge = median(raw_data$age)
medianDuration = median(raw_data$duration)
...
```{r}
ggplot(final_cont, aes(x = duration)) + geom_density() + facet_wrap(~set,nrow =2)
```

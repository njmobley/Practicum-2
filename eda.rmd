...
formulas <- gen_formula <- function(picks,curfeatures = c("default","education","loan","age")) {
  formula(str_c('y ~',str_c(curfeatures[picks], collapse = '+')))
}
...
create_glm <- function(formula) {
  train(formula,data = training, trainControl = train_control,method = "glm",family = binomial())
}


full_model = glm(y ~ default + loan + education + age, data = training, family = binomial())
predictions = predict(full_model,testing, type = 'response')
```

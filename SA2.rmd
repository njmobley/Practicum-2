...
ggplot(data=raw_data, aes(x=campaign)) + geom_density()
ggplot(data=raw_data, aes(x=log(campaign))) + geom_density()
ggplot(data=raw_data, aes(x=sqrt(campaign))) + geom_density()
ggplot(data=raw_data, aes(x=campaign^(-.57))) + geom_density()

# We can try Box Cox
bc = powerTransform(cbind(previous +.1, campaign, pdays+1.1 , day) ~ 1, raw_data)
summary(bc)
library(BBmisc)
campaign2 <- normalize(raw_data$campaign, method = "standardize", range = c(0, 1), margin = 1L, on.constant = "quiet")
...
## 2.3  Build a logistic regression model with the binary response variable ifrespondents is willing to purchase the product, and include all of theother variables as explanatory variables.  Perform model selection tochoose the optimal model.
```{r logisticregressionmodels}

```


## 2.4.  In order to determine whether the campaign should move forward,estimate  the  overall  positive  response  rate  and  compare  it  to  thebreak-even response rate of 24.13%.
